...
def percentile_rank(values: List[Optional[float]]) -> List[float]:
    """Return a 0..1 percentile rank for each value. None -> 0.0 (lowest)."""
...
def score_candidates(
    ...
    for i, f in enumerate(features):
        # --- group: compression / VCP (already 0..1 signals) ---
        ...
        squeeze_bonus = 0.10 if f.get("squeeze") else 0.0
        compression_vcp = min(
            1.0, 0.4 * compression + 0.3 * vcp + 0.15 * nr + 0.15 * vdu + squeeze_bonus
        )
        ...
        # --- group: dealer GEX (UW, already 0..1) ---
