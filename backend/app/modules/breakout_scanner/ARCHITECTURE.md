...
class SmartMoneyProvider(Protocol):
    ...
    def greek_exposure_series(self, ticker: str, timeframe: str = "1M") -> list[dict]: ...
    def flow_per_expiry(self, ticker: str) -> list[dict]: ...
    def darkpool_price_levels(self, ticker: str) -> dict: ...
    def insider_ticker_flow(self, ticker: str, limit: int = 50) -> list[dict]: ...
...
> **Consumed outside this module:** `greek_exposure_series`, `flow_per_expiry`,
> `darkpool_price_levels` and `insider_ticker_flow` exist for the app's Flow &
> Positioning score (`backend/app/services/direction.py`), not for `run_scan`.
> They live here so there is one Unusual Whales client with one set of auth,
> retry and throttle rules. Scans do not call them.
...
- `UnusualWhalesProvider` — `httpx` REST client. Per-instance response cache, **429
  ...
  **Note:** the client is synchronous and its retry/throttle paths use `time.sleep`,
  so an async caller must wrap it in `asyncio.to_thread` (see `direction.py`).
