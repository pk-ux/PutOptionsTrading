...
async def screen_options(
    ...
    if user and enforce_limits:
        ...
        if len(screen_request.symbols) > max_symbols:
            raise HTTPException(
                status_code=400,
    ...
    results, used_yahoo = screen_symbols(
        symbols=screen_request.symbols,
        max_dte=screen_request.max_dte,
        min_dte=screen_request.min_dte,
        min_volume=screen_request.min_volume,
        min_open_interest=screen_request.min_open_interest,
        min_annualized_return=screen_request.min_annualized_return,
        max_assignment_probability=screen_request.max_assignment_probability
    )
